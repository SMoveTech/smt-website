...
function renderDocsBox(p) {
  ...
  const items = p.docs.map(d => {
    ...
    return `<a class="docitem" href="/build/doc/${esc(d.id)}" target="_blank" rel="noopener">
      ...
      <span class="doc-info"><span class="doc-title">${esc(d.title)} ↗</span>${d.note ? `<span class="doc-note">${esc(d.note)}</span>` : ''}</span></a>`;
  }).join('');
  return `<div class="box full"><h3>Technical documentation</h3>
    <p class="lead">Opens in a new tab.</p><div class="doclist">${items}</div></div>`;
}
...
module.exports = { renderDashboard, renderLogin, renderNotApproved, renderEnrolled, renderDevices, renderChangePassword, renderDocPage };
